...
- **Lifelines:** start with 3 — **50:50, Ask the Audience, Phone a Friend** — one use each per run. Buy a **permanent second slot per type**; **cap 2 per type** (max 6 uses/run).
  ...
  - **Ask the Audience:** a real, fallible poll — the correct answer draws the most weight on average (so it **helps**), but a "trap" distractor can occasionally win, more often on hard. Clear/wide on easy, thin and sometimes wrong on hard. *(Owner revision of the original "plurality never wrong" — see `docs/LIFELINES.md`.)*
  - **Phone a Friend:** a ~10s cutscene of a panicking friend who names a guess — **68% correct / 32% wrong** (`PHONE_ACCURACY`). Panic dialogue is generic UI flavour, not exam content. *(Owner revision of the original "always hedges toward the correct option, authored content" — see `docs/LIFELINES.md`.)*
- **Integrity of assists:** a lifeline-assisted **correct** answer does **not** promote mastery. Lifelines **advise, never grade** — the authored key alone decides correctness (Audience/Phone may now point at a wrong option; they are hints, not verdicts).
- **No player timers:** there is never a countdown to answer; the only deliberately timed sequence is the Phone-a-Friend cutscene (`docs/LIFELINES.md`).
